...
interface DateFlightMenuProps {
    today: Date,
    onChangeStartDateDisplay: (value: string) => void,
    onChangeEndDateDisplay: (value: string) => void,
    onChangeStartDate: (value: Date | null) => void,
    onChangeEndDate: (value: Date | null) => void
}

export default function DateFlightMenu({ today, onChangeStartDateDisplay, onChangeEndDateDisplay, onChangeStartDate, onChangeEndDate }: DateFlightMenuProps) {
  ...
  function handleDayClick(date: Date) {
    if (!startDate || (startDate && endDate)) {
        setStartDate(date);
        setEndDate(null);
        onChangeStartDateDisplay(formatDate(date))
        onChangeStartDate(date)
        onChangeEndDateDisplay("")
        onChangeEndDate(null)
    } else {
      if (isSameDay(date, startDate)) {
        setStartDate(null);
        onChangeStartDateDisplay("")
      ...
      if (date < startDate) {
        ...
        onChangeStartDate(date)
        onChangeStartDateDisplay(formatDate(date))
        onChangeEndDateDisplay(formatDate(startDate))
      } else {
        setEndDate(date);
        onChangeEndDate(date);
        onChangeEndDateDisplay(formatDate(date))
      }
    }
  }
        ...
        <div style={{ marginTop: 28, paddingTop: 20, borderTop: "1px solid #eef1f7", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {(startDate || endDate) && (
            <button onClick={() => { 
                ...
                onChangeStartDate(null)
                onChangeEndDate(null)
              }} style={{ fontSize: 13, color: "#8a94a8", background: "none", border: "none", cursor: "pointer", padding: "6px 12px", borderRadius: 8, transition: "color 0.15s" }}
